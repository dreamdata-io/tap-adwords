...
GENERIC_ENDPOINT_MAPPINGS = {
    "campaigns": {"primary_keys": ["id"], "service_name": "CampaignService"},
    "ad_groups": {"primary_keys": ["id"], "service_name": "AdGroupService"},
    "ads": {"primary_keys": ["adGroupId"], "service_name": "AdGroupAdService"},
    "accounts": {
...
REPORTS_WITH_90_DAY_MAX = frozenset(["CLICK_PERFORMANCE_REPORT"])
...
def get_end_date():
    ...
    return datetime.datetime(today.year, today.month, today.day).astimezone()
...
def write_schema(stream_name, schema, primary_keys, bookmark_properties=None):
    ...
    singer.write_schema(
        stream_name, schema_copy, primary_keys, bookmark_properties=bookmark_properties
    )
...
def add_synthetic_keys_to_stream_schema(stream_schema):
    stream_schema["properties"]["_sdc_customer_id"] = {
        "description": "Profile ID",
        "type": "string",
        "field": "customer_id",
    }
    stream_schema["properties"]["_sdc_report_datetime"] = {
...
def sync_report(stream: str, field_list: List[str], sdk_client: adwords.AdWordsClient):
    ...
    while start_date <= get_end_date():
        sync_report_for_day(
            stream, field_selector, field_reverse, sdk_client, start_date
        )
    ...
    bookmarks.clear_bookmark(
        STATE, stream, state_key_name(customer_id, "last_attribution_window_date")
    )
    singer.write_state(STATE)
...
def get_xml_attribute_headers(stream_schema, description_headers):
    ...
    xml_attribute_headers = [
        description_to_xml_attribute[header] for header in description_headers
    ]
...
def attempt_download_report(report_downloader, report):
    try:
        ...
        if isinstance(
            e, AdWordsReportBadRequestError
        ) and "RateExceededError.RATE_EXCEEDED" in str(e):
            raise RateExceededError(
                "Rate Exceeded Error. Too many requests were made to the API in a short period of time."
            ) from e
        raise


def sync_report_for_day(
    stream_name, field_list, field_reverse, sdk_client, start
    ...
    with metrics.record_counter(stream_name) as counter:
        ...
        if start > get_start_for_stream(sdk_client.client_customer_id, stream_name):
            LOGGER.info(
                ...
                start,
                get_start_for_stream(sdk_client.client_customer_id, stream_name),
            )
            bookmarks.write_bookmark(
                STATE,
                stream_name,
...
CAMPAIGNS_BLACK_LISTED_FIELDS = set(
    ["networkSetting", "conversionOptimizerEligibility", "frequencyCap"]
)
...
@with_retries_on_exception(
    RETRY_SLEEP_TIME, MAX_ATTEMPTS, dont_retry=[CustomerNotActiveError]
)
def attempt_get_from_service(service_caller, selector):
    try:
        ...
        LOGGER.info(
            "An exception was thrown in %s with selector: %s",
        ...
        if isinstance(e, GoogleAdsServerFault) and "CUSTOMER_NOT_ACTIVE" in str(e):
            raise CustomerNotActiveError(
                "Customer Not Active AuthorizationError. This usually means that you haven't been active on your account for 15 months."
            ) from e
        raise
...
def get_page(sdk_client, selector, stream, start_index):
    ...
    with metrics.http_request_timer(stream):
        LOGGER.info(
            "Request %s %s for customer %s with startIndex %s using selector %s",
            PAGE_SIZE,
            stream,
...
def iter_safe_selectors(selector, predicate_field, kosher_fn):
    ...
    while predicate_values:
        to, success = binary_search(
            predicate_values, 0, len(predicate_values) - 1, kosher_fn
        )
        selector = set_selector_predicate_values(
            selector, predicate_field, predicate_values[0 : to + 1]
        )
...
def get_predicate(selector, predicate_field):
    return [p for p in selector["predicates"] if p["field"] == predicate_field][0]
...
def get_campaign_ids_selector(campaign_ids, fields, start_index):
    return {
        ...
        "paging": {"startIndex": str(start_index), "numberResults": str(PAGE_SIZE)},
    }
...
def get_campaign_ids_safe_selectors(sdk_client, campaign_ids, fields, stream):
    LOGGER.info(
        "Discovering safe %s selectors for customer %s",
        stream,
        sdk_client.client_customer_id,
    )

    is_campaign_ids_safe_lambda = lambda cids: is_campaign_ids_safe(
        sdk_client, stream, cids
    )
    start = get_campaign_ids_selector(campaign_ids, fields, 0)
    if is_campaign_ids_safe_lambda(campaign_ids):
        yield start, True

    yield from iter_safe_selectors(start, "BaseCampaignId", is_campaign_ids_safe_lambda)
...
def get_selector_ids(sdk_client, stream, selector):
    LOGGER.info(
        "Retrieving selector ids for customer %s, selector hash %s",
        sdk_client.client_customer_id,
        hash(str(selector)),
    )
    service_name = GENERIC_ENDPOINT_MAPPINGS[stream]["service_name"]
    ...
    while True:
        ...
        LOGGER.info(
            "Request %s selector ids for customer %s, selector hash %s",
            PAGE_SIZE,
            sdk_client.client_customer_id,
            hash(str(selector)),
        )
        ...
        if page["totalNumEntries"] > GOOGLE_MAX_START_INDEX:
            raise Exception(
                "Too many entries (%s > %s) for customer %s, selector hash %s"
                % (
        ...
        if offset > int(page["totalNumEntries"]):
            break
    LOGGER.info(
        "Retrieved %s selector ids for customer %s. Expected %s.",
        len(selector_ids),
        sdk_client.client_customer_id,
        page["totalNumEntries"],
    )
...
def get_ad_group_ids_selector(campaign_ids_selector, ad_group_ids):
    ...
    campaign_ids_selector_copy["predicates"].append(
        {"field": "AdGroupId", "operator": "IN", "values": ad_group_ids}
    )
...
def is_selector_safe(sdk_client, stream, selector, predicate_field):
    ...
    if (
        len(get_predicate_field_values(selector, predicate_field))
        > GOOGLE_MAX_PREDICATE_SIZE
    ):
        LOGGER.info(
            "Selector is unsafe: length of %s values exceeds %s",
            predicate_field,
            GOOGLE_MAX_PREDICATE_SIZE,
        )
        return False
...
def get_ad_group_ids_safe_selectors(sdk_client, campaign_ids_selector, stream):
    if len(campaign_ids_selector["predicates"][0]["values"]) > 1:
        raise Exception(
            "Cannot select ad_group_ids when more than one campaign is used"
        )
    ...
    ad_group_ids = list(
        get_selector_ids(sdk_client, "ad_groups", campaign_ids_selector)
    )

    for selector, success in iter_safe_selectors(
        get_ad_group_ids_selector(campaign_ids_selector, ad_group_ids),
        "AdGroupId",
        lambda agids: is_ad_group_ids_safe(
            sdk_client, stream, campaign_ids_selector, agids
        ),
    ):
        if not success:
        ...
        page = get_page(sdk_client, selector, stream, 0)
        total_num_entries_dict["selector_" + str(hash(str(selector)))] = page[
            "totalNumEntries"
        ]
        yield selector
...
def get_safe_selectors(sdk_client, campaign_ids, fields, stream):
    for campaign_id_selector, success in get_campaign_ids_safe_selectors(
        sdk_client, campaign_ids, fields, stream
    ):
        if success:
            yield campaign_id_selector
        else:
            for selector in get_ad_group_ids_safe_selectors(
                sdk_client, campaign_id_selector, stream
            ):
                yield selector
...
def sync_campaign_ids_endpoint(sdk_client, campaign_ids, stream, stream_metadata):
    ...
    for selector in get_safe_selectors(
        sdk_client, list(campaign_ids), field_list, stream
    ):
        ...
        while True:
            ...
            if page["totalNumEntries"] > GOOGLE_MAX_START_INDEX:
                raise Exception(
                    "Too many {} ({} > {}) for customer {}, selector {}".format(
                        stream,
                        GOOGLE_MAX_START_INDEX,
                        page["totalNumEntries"],
                        sdk_client.client_customer_id,
                ...
                with metrics.record_counter(stream) as counter:
                    ...
                    with Transformer(
                        ...
                        for obj in page["entries"]:
                            ...
                            singer.write_record(
                                stream, record, time_extracted=time_extracted
                            )
                            counter.increment()
            ...
            if start_index > int(page["totalNumEntries"]):
                break
    if total_num_entries_dict.get("baseCampaignId") and total_num_entries_dict[
        "baseCampaignId"
    ] != sum(
        [v for k, v in total_num_entries_dict.items() if k.startswith("selector_")]
    ):
        sum_of_selectors = sum(
            [v for k, v in total_num_entries_dict.items() if k.startswith("selector_")]
        )
        LOGGER.warning(
            "A difference was found between totalNumEntries of a search using just BaseCampaignId and one using BaseCampaignId and AdGroupId's."
        )
    ...
    LOGGER.info(
        "Done syncing %s for customer_id %s", stream, sdk_client.client_customer_id
    )
...
def get_and_cache_parent_account_tz_str(sdk_client):
    ...
    if sdk_client.client_customer_id not in parent_account_tz_cache:
        ...
        if results["totalNumEntries"] == 1:
            parent_account_tz_cache[sdk_client.client_customer_id] = results["entries"][
                0
            ]["dateTimeZone"]
        elif results["totalNumEntries"] == 0:
            LOGGER.warning(
                (
                    "Google returned an empty managed customer "
                    ...
                    for obj in page["entries"]:
                        ...
                        if stream == "campaigns":
                            ...
                            if record.get("startDate"):
                                ...
                                utc_date = (
                                    pytz.timezone(parent_account_tz_str)
                                    .localize(naive_date)
...
def sync_generic_endpoint(stream_name, stream_metadata, sdk_client):
    if stream_name in ("ads", "ad_groups"):
        selector = {
            "fields": ["Id"],
            "paging": {"startIndex": str(0), "numberResults": str(PAGE_SIZE)},
        }
        campaign_ids = get_selector_ids(sdk_client, "campaigns", selector)

        if not campaign_ids:
            LOGGER.info(
                "No %s for customer %s", stream_name, sdk_client.client_customer_id
            )
            return

        sync_campaign_ids_endpoint(
            sdk_client, campaign_ids, stream_name, stream_metadata
        )
    elif stream_name in ("campaigns", "accounts"):
        sync_generic_basic_endpoint(sdk_client, stream_name, stream_metadata)
    else:
...
def create_field_metadata_for_report(stream, fields, field_name_lookup):
        ...
        if hasattr(field, "exclusiveFields"):
            mdata = metadata.write(
                ...
                [
                    ...
                    for x in field["exclusiveFields"]
                ],
            )
        mdata = metadata.write(mdata, breadcrumb, "behavior", field["fieldBehavior"])
        mdata = metadata.write(
            mdata, breadcrumb, "adwords.fieldName", field["fieldName"]
        )
    ...
    if stream == "GEO_PERFORMANCE_REPORT":
        ...
        mdata = metadata.write(
            mdata, ("properties", "countryTerritory"), "inclusion", "automatic"
        )
...
def create_schema_for_report(stream, sdk_client):
    ...
    for field in fields:
        ...
        report_properties[field["xmlAttributeName"]] = {
            "description": field["displayFieldName"]
        }
        report_properties[field["xmlAttributeName"]].update(
            create_type_map(field["fieldType"])
        )
    ...
    return (
        {"type": "object", "is_report": "true", "properties": report_properties},
        mdata,
    )
...
def check_selected_fields(stream, field_list, sdk_client):
    ...
    for field in fields:
        ...
        if field_errors:
            errors.append(
                "{} cannot be selected with {}".format(
    ...
    if errors:
        if sdk_client.user_agent == "Stitch Tap (+support@stitchdata.com)":
            ...
            advice_message = (
                "You should correct it in the catalog being passed " "to the tap."
            )
        raise Exception(
...
def do_discover_reports(sdk_client):
    url = "https://adwords.google.com/api/adwords/reportdownload/{}/reportDefinition.xsd".format(
        VERSION
    )
    xsd = request_xsd(url)
